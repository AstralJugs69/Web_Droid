...
import { PreviewStateGetter } from './AppContainer';
...
// Render the specific app component based on appId
...
const DeviceFrame: React.FC<DeviceFrameProps> = ({ 
  ...
  // Frame styles based on device type
  const frameStyles = deviceType === 'phone'
    ? 'p-3'
    : 'p-4';
  ...
  // Render the active app based on its ID
  const renderActiveApp = () => {
    // Don't render anything if there's no current app or if we're not unlocked
    if (!currentApp || bootState !== 'unlocked') return null;

    // IMPORTANT: Don't render the app twice
    // The app is already rendered in renderPersistentApps(), so we don't need another instance here
    return null;
  };
    ...
    <div className={`h-screen w-screen flex flex-col items-center justify-center bg-background`}>
      ...
      >
        ...
        >
          ...
          {/* Don't render active app a second time - it's already in renderApps() */}
          {/* {bootState === 'unlocked' && renderActiveApp()} */}
